...
User.add({
	...
	isAdmin: { type: Boolean, label: 'Can Admin ShanghaiJS' },
...
User.schema.virtual('avatarUrl').get(function() {
	...
	if (this.gravatar) return 'http://www.gravatar.com/avatar/' + this.gravatar + '?d=http%3A%2F%2FShanghaiJS.com%2Fimages%2Favatar.png&r=pg';
...
User.schema.methods.resetPassword = function(callback) {
	...
	user.save(function(err) {
		...
		new keystone.Email('forgotten-password').send({
			...
			subject: 'Reset your ShanghaiJS Password',
			to: user.email,
			from: {
				name: 'ShanghaiJS',
